...
// Financial state is private to the authenticated account. The legacy key is
// migrated once for the first account that opens the upgraded app, then removed.
const LEGACY_STORAGE_KEY = 'vigil-state-v3';
const ACCOUNT_STORAGE_PREFIX = 'vigil-state-v4:';
...
const copy: Record<Language, Record<string, string>> = {
  en: {
    ...
    support: 'Support', report: 'Your monthly report', reportCopy: 'Vigil is spotting the patterns so your money can get a better job.',
    unlock: 'Unlock deeper insights', locked: 'Premium reports are ready when you are.', manage: 'Manage subscription',
    buckets: 'Your buckets', bucketCopy: 'Give every dirham a job before it gets a chance to disappear.', development: 'Self Development',
  ...
  fr: {
    ...
    terms: 'Conditions', support: 'Assistance', report: 'Votre rapport mensuel', reportCopy: 'Vigil repère vos habitudes pour aider votre argent.',
    ...
    bucketCopy: 'Donnez un rôle à chaque dirham avant qu’il ne disparaisse.', adjust: 'Modifier', total: 'Total', saved: 'Enregistré',
  ...
  de: {
    ...
    report: 'Dein Monatsbericht', reportCopy: 'Vigil erkennt Muster, damit dein Geld besser arbeitet.', unlock: 'Tiefere Einblicke freischalten',
  ...
  es: {
    ...
    report: 'Tu informe mensual', reportCopy: 'Vigil encuentra patrones para que tu dinero trabaje mejor.', unlock: 'Desbloquear más información',
  ...
  ru: {
    ...
    report: 'Ваш отчёт за месяц', reportCopy: 'Vigil ищет закономерности, чтобы ваши деньги работали лучше.', unlock: 'Открыть глубокий анализ',
  ...
  ar: {
    ...
    report: 'تقريرك الشهري', reportCopy: 'يرصد Vigil الأنماط لتعمل أموالك بشكل أفضل.', unlock: 'فتح التحليلات المتقدمة', locked: 'التقارير المميزة جاهزة لك.',
    manage: 'إدارة الاشتراك', buckets: 'فئاتك', development: 'التطوير الذاتي', bucketCopy: 'امنح كل درهم مهمة قبل أن يختفي.', adjust: 'تعديل', total: 'الإجمالي', saved: 'تم الحفظ',
...
export function VigilProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!raw) {
        // Existing installs used one device-wide key before account isolation
        // existed. Claim it for the account active during this upgrade only.
  ...
  const refreshRates = async (): Promise<Partial<Record<CurrencyCode, number>>> => {
    try {
      ...
      const data = await response.json() as { rates?: Partial<Record<CurrencyCode, number>>; time_last_update_utc?: string };
      if (!data.rates?.AED) throw new Error('Invalid rates response');
      const nextRates = { ...data.rates, AED: 1 };
      setState((current) => ({
        ...current,
  ...
  useEffect(() => {
    ...
    const stale = !state.ratesUpdatedAt || Date.now() - new Date(state.ratesUpdatedAt).getTime() > 12 * 60 * 60 * 1000;
  ...
  const value = useMemo<AppContextValue>(() => ({
    ...
    t: (key: string) => {
      ...
      const userFacingText = text.replaceAll('RevenueCat', subscriptionProviderLabels[state.language]);
    ...
    addIncome: (amount) => setState((current) => ({ ...current, income: current.income + amount })),
    ...
    formatMoney: (amountInAed) => {
      const rate = scopedState.rates[scopedState.currency] ?? 1;
    ...
    toBaseAmount: (displayAmount) => displayAmount / (scopedState.rates[scopedState.currency] ?? 1),
